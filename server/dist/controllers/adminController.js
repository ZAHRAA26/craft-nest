...
async function listUsers(_req, res) {
    const users = await userService_1.userService.all();
    const safe = users.map((user) => ({
        id: user._id,
        name: user.name,
        email: user.email,
        role: user.role === roles_1.UserRole.ADMIN ? undefined : user.role,
...
async function updateRole(req, res) {
    ...
    user.role = roles_1.RoleMapping[role];
    await user.save();
 ...
 */
async function revokeRefresh(req, res) {
    const { id } = req.params;
    const { jti } = req.body;
    if (!id) {
    ...
    if (!jti) {
        await userService_1.userService.revokeAllRefreshTokens(id);
        return res.json({ message: req.t('admin:all_refresh_tokens_revoked') });
    }
    await userService_1.userService.removeRefreshToken(id, jti);
...
async function sendNewsletter(req, res) {
    ...
    await Promise.all(recipients.map((email) => (0, mailer_1.sendNewsEmail)(email, subject, message)));
    return res.json({
        success: true,
