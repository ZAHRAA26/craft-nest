...
async function logout(req, res) {
    ...
    await authService_1.authService.logout(userReq._id.toString(), refreshToken, allDevices);
...
async function getMe(req, res) {
    const userReq = req.user;
    const user = await userService_1.userService.findById(userReq._id.toString());
    if (!user) {
    ...
    return res.json({
        ...
        name: user.name,
        email: user.email,
        role: (0, roles_1.getPublicRole)(user.role), // إظهار الاسم العام بدلاً من المفتاح السري
...
async function register(req, res) {
    ...
    await authService_1.authService.register(req.body, baseUrl);
...
async function refreshToken(req, res) {
    ...
    const accessToken = await authService_1.authService.refreshToken(refreshToken);
...
async function forgotPassword(req, res) {
    const baseUrl = `${req.protocol}://${req.get('host')}`;
    await authService_1.authService.forgotPassword(req.body.email, baseUrl, req.language);
...
async function verifyEmail(req, res) {
    ...
    await authService_1.authService.verifyEmailToken(token, req.language);
